...
会。扩展使用 Chromium `idle` 权限监测电脑状态：连续 60 秒没有键盘或鼠标输入，或者系统进入锁定状态时，正在运行的自动投递会在当前步骤结束后暂停，不再开始下一个岗位；电脑恢复为活动状态后会从已保存的进度自动继续。

这里检测的是系统“无输入/锁定”状态，而不是显示器硬件的电源信号，因此不同系统的熄屏时机可能略有差异。用户手动点击的暂停优先级更高，不会因为鼠标再次活动而被自动恢复。电脑真正休眠期间浏览器本身不会执行任务，唤醒后则按上述规则恢复。
