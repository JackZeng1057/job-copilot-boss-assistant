...
达标岗位会在当前职位详情中识别 BOSS 原生“立即沟通”或“继续沟通”按钮，并且只点击一次。

- 若 BOSS 显示“留在此页”，扩展点击该选项并继续处理岗位。
- 若 BOSS 仍把当前标签带到消息页，扩展使用浏览器后退返回原职位列表；后退不可用时才用已保存的职位列表 URL 恢复。
- 只有正在执行的沟通点击可以触发自动返回；用户或页面因其他原因离开职位页时，自动投递仍会暂停。
- 不创建临时沟通标签，不重复点击沟通按钮。
