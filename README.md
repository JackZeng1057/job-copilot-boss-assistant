...
4. **额外分析提示词**：补充个性化条件和评分侧重点，例如接受应届岗位、优先工作内容等。提示词只影响当前用户的 AI 分析，不会写入插件源码，也不会带给其他安装者。
5. **话术风格**：控制达标岗位生成的沟通表达，例如“简洁、真诚，突出相关项目和到岗意愿”。不要在这里预先写死某家公司或某个岗位名称。
...
- “立即沟通”只在不激活的临时标签中点击；无论 BOSS 留在详情页还是进入消息页，原职位列表标签都不会被导航。
- 临时标签完成结果核验后立即关闭，不会抢占当前标签的焦点。
...
AI 综合求职配置、所有已勾选简历、完整 JD、岗位门槛、地点和机会质量给出最终 0-100 分。扩展只校验分数范围、执行用户明确配置的排除结论，并与最低分比较，不使用固定职业名单替所有用户做决定。
...
- `storage`：保存本机配置和运行状态。
- `tabs`：打开独立消息标签，并在自动沟通意外进入消息页时返回原职位列表。
