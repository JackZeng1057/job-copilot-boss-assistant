...
> Edge 与 Chrome 的扩展按钮位置可能略有差异。若工具栏没有显示 `J` 图标，请重新打开扩展菜单并确认 Job Copilot 已固定；不要在网页中的账号头像附近寻找插件入口。
...
- `目标城市/通勤回答`：填写城市、区域或可接受的通勤方式。
- `目标方向/关键词`：填写希望寻找的岗位方向，不要照搬某一个人的配置。
- `额外分析提示词`：补充个人接受和不接受的条件。
- `话术风格`：控制生成的首句沟通表达。
...
- TXT/Markdown：直接读取文本。
- PDF：使用随扩展打包的 PDF.js 和本地 CMap 资源解析，不上传到第三方文件服务。
- 扫描件/图片型 PDF：PDF.js 无法进行 OCR，需要先自行 OCR，或将正文复制到文本框。
...
当前版本不会主动识别系统休眠、锁屏或仅关闭显示器，也不会把面板状态自动切换成“已暂停”。电脑真正休眠时，浏览器脚本通常会被操作系统挂起，但唤醒后定时任务可能继续；仅熄屏时，浏览器甚至可能一直运行。

因此不要把休眠或熄屏当作暂停功能。离开电脑前请在 Job Copilot 面板中手动点击“暂停自动投递”，返回后确认页面和账号状态正常，再点击继续。后续若加入系统空闲检测，需要单独申请 Chromium `idle` 权限并处理锁屏、休眠和唤醒恢复，目前尚未实现。
