...
Chrome 与 Edge 的扩展管理页布局略有不同，但安装流程和所选目录完全相同。

### 4. 固定扩展并完成首次配置

1. 点击浏览器工具栏的扩展图标，将 `Job Copilot BOSS 助手` 固定到工具栏。
2. 打开插件，填写自己的 AI 服务、API Key、模型 ID、求职偏好和简历。
3. 点击“保存”，并在浏览器询问时允许访问所填写的 AI 接口域名。
4. 登录 BOSS 直聘并进入职位列表页，点击页面右侧的 `JC` 按钮开始使用。
