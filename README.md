...
- 岗位级进度列表展示待分析、分析中、达标、未达标、已沟通和失败状态；可点击岗位右侧的 `×` 提前关闭检测并将其移出本次投递列表。
...
3. **绝不投递岗位/职业类型**：填写当前用户明确排除的完整职业类型，例如“电话销售，保险销售，直播运营”。默认留空。避免只写“销售、运营、支持”等边界过宽的词；AI 会根据岗位的完整职业语义判断，不能因为共享单个词就排除相邻岗位。
4. **工作经验年限与是否应届**：经验年限可留空；应届身份可选“未设置 / 应届生 / 非应届生”。AI 会对照职位卡片和完整 JD 中的“经验不限、3-5 年、应届”等要求，但该因素只占总分 6 分，不会单独淘汰岗位。
5. **额外分析提示词**：补充个性化条件和评分侧重点，例如接受应届岗位、优先工作内容等。提示词只影响当前用户的 AI 分析，不会写入插件源码，也不会带给其他安装者。
6. **话术风格**：控制达标岗位生成的沟通表达，例如“简洁、真诚，突出相关项目和到岗意愿”。不要在这里预先写死某家公司或某个岗位名称。
...
- 所有支持的 AI 协议统一使用紧凑评分 Schema 和输入长度预算；对允许省略输出上限的协议沿用服务商/模型默认值，Anthropic Messages 仅保留接口强制要求的 `max_tokens`。OpenAI 兼容、Responses、Anthropic 与 Gemini 分别启用原生 JSON/结构化输出能力。
- 批量评分使用协议能力层降低可选推理强度（例如 DeepSeek V4 关闭 thinking、Gemini Flash 使用低延迟思考配置），不影响其他不支持该参数的厂商。
- AI 后台新增统一“分析速度模式”：极速关闭或降低推理，平衡使用少量推理，精确使用较高推理或服务商默认；未知模型和自定义兼容接口不发送猜测参数。
- 岗位条目在“定位”左侧新增“重新分析”。它复用该岗位最近一次采集的完整 JD 输入并行执行，不暂停主队列、不改变序号，也不会自动沟通。
- token 用量仅用于本地诊断日志和性能测试，不在插件岗位列表中显示。
- 尾逗号、单引号、未加引号键、Python 布尔值、代码围栏、额外说明、字符串控制字符和普通结构残缺会在本地修复，避免为格式问题再次等待模型；若服务商明确声明输出因 token 上限截断，则安全停止该岗位，不使用半份评分。
- 达标后沟通过渡为约 3 秒，岗位之间保留约 5 秒节奏；实际总耗时还取决于 JD 读取和 AI 响应。
- 自动化日志新增 JD 读取耗时、AI 服务商耗时和本地 JSON 容错标记；不再发送第二次模型修复请求，避免额外延迟和 token 消耗。

### 0.9.0 更新

- 每个岗位进度条目右侧新增 `×` 按钮，可在检测或等待沟通阶段提前关闭该岗位。
- 关闭后岗位会立即移出当前投递列表，剩余岗位序号自动连续重排；迟到的 AI 分析结果不会继续触发沟通，自动投递会接着处理下一个岗位。
- 已进入实际沟通步骤的岗位会暂时禁用关闭按钮，避免出现沟通结果不明确的状态。
...
4. 点击“开始自动投递”。
5. AI 逐个读取完整岗位信息并评分。
6. 达标岗位等待约 3 秒后沟通，每个岗位结束后等待约 5 秒再继续。
...
达标岗位会先在当前职位详情中识别 BOSS 原生“立即沟通”或“继续沟通”按钮。

- 职位列表标签和消息标签是强制隔离的两个浏览器标签；插件不允许职位标签承载 `/web/geek/chat`。
- 自动“立即沟通”使用当前职位列表标签里已经加载的 BOSS 原生详情按钮，并保留页面真实点击事件；不会为了沟通新开职位详情标签。
- 若 BOSS 异步把受保护职位标签带入消息页或详情页，内容脚本和后台会多阶段复查并恢复；只有已经没有可用浏览器历史时，才在同一标签恢复保存的职位列表 URL。
- “继续沟通”表示该岗位已有会话，扩展直接记录为已沟通，不再点击进入消息页。
- 每个岗位最多触发一次沟通点击，不会因结果确认超时而重复发送。
- 用户或页面主动离开职位页时，自动投递仍会暂停，不会擅自刷新或改写职位列表。
- 若 BOSS 在读取 JD 时通过 SPA 路由将列表标签带到职位详情页，扩展会使用浏览器历史返回并暂停，保留原列表筛选和运行进度。
- 在职位列表页手动点击某岗位的“立即沟通”时，扩展保留这次真人可信点击给 BOSS 原生处理，不再吞掉后用合成点击重放；出现“已发送 / 留在此页”弹窗时自动选择留在此页。只有成功弹窗、按钮变成“继续沟通/再次沟通”等明确证据出现后，才从队列精确移除该岗位并重新编号。无法确认 BOSS job ID、结果不明确或失败时一律保留岗位。
- 手动点击已有沟通记录的“继续沟通/再次沟通”时，只在独立消息标签中打开会话，职位列表标签保持不变。
- 如果 BOSS 的其他 SPA 行为仍把受保护的职位标签带入消息页，后台会自动执行一次受控恢复并暂停当前批次；该保护在自动投递尚未开始时也生效。

用户手动点击 BOSS 顶部“消息”时，会复用已有消息标签；若不存在，扩展会新建独立消息标签。原职位标签和其中的岗位列表、分析进度不会被修改。

## 评分与排除系统

AI 综合求职配置、所有已勾选简历、完整 JD、岗位门槛、地点和机会质量给出最终 0-100 分。扩展只校验分数范围、执行用户明确配置的排除结论，并与最低分比较，不使用固定职业名单替所有用户做决定。岗位门槛中的工作经验年限与应届身份合计占 0-6 分，用于小幅调整而非自动淘汰。
...
以下配置保存在 `chrome.storage.local`：

- API Base URL、API Key 和模型名。
- 求职偏好和开关。
- 简历正文及导入文件名。
- 最近最多 200 条本地运行日志；日志不包含完整模型 JSON、简历正文或 API Key。

自动化进度优先保存在内存型 `chrome.storage.session`，扩展重载、更新、禁用或浏览器重启后会清除；旧环境不支持该接口时才回退到 `storage.local`。会话中最多保留 50 个岗位的精简判断字段。AI 服务返回的原始 JSON 只在当前请求解析期间存在，不会作为文件或存储项保留。

token 统计只保存服务商响应中的数字（输入、可见输出、思考、缓存和总 token）及耗时，不保存服务商原始响应，也不在插件面板展示。
...
- `storage`：保存本机配置和运行状态。
- `tabs`：打开独立消息/临时沟通标签，并在职位标签意外进入消息页或职位详情页时自动恢复原职位列表。
- `debugger`：只在自动执行当前 BOSS 职位页的“立即沟通”时，通过 Chromium 调试协议发送一次浏览器可信鼠标点击。扩展会先校验页面、岗位、按钮位置和遮挡状态，仅使用鼠标输入能力，并在点击完成或失败后立即断开调试连接。
...
自动“立即沟通”需要产生 BOSS 页面能够识别的浏览器可信点击，普通脚本生成的合成点击无法稳定完成这一操作，因此扩展声明了 Chromium 的 `debugger` 权限。

该权限只用于当前 BOSS 职位标签中的一次鼠标移动、按下和释放。执行前会校验发起标签仍是 BOSS 职位列表、目标岗位 URL 有效、页面地址未变化、按钮位于可点击视口且没有被 BOSS 登录或安全验证等页面元素遮挡。点击结束或任何步骤失败时都会立即断开调试连接；扩展不会通过该权限读取网页网络流量、浏览器 Cookie、密码或其他标签页内容。
