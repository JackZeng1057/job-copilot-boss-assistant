// 配置表单、权限申请和简历导入状态；文件格式解析由 popup-resume.js 负责。
import { readResumeFile } from "./popup-resume.js";
...
document.getElementById("save").addEventListener("click", async () => {
  ...
  const scoreInput = Number(fields.minScore.value || defaults.minScore);
  const minScore = Number.isFinite(scoreInput)
  ...
  chrome.storage.local.set(
    ...
    () => {
      ...
      if (error) {
        alert(`保存失败：${error.message || "浏览器存储不可用"}`);
        return;
      }
      window.close();
    }
...
for (const fieldName of Object.keys(resumeFileNames)) {
  fields[`${fieldName}File`].addEventListener("change", () => importResumeFile(fieldName, fields[`${fieldName}File`]));
  fields[fieldName].addEventListener("input", () => markResumeTextEdited(fieldName));
}
